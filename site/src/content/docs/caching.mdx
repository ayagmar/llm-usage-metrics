...
| event store | stores parsed file diagnostics/events keyed by file fingerprint | no TTL | `<platform-cache-root>/llm-usage-metrics/events.db` |
...
- delete the database file to force a full rebuild on the next run
...
- For stale reports after source file changes, verify source files updated `mtime` and delete `<platform-cache-root>/llm-usage-metrics/events.db` to rebuild from scratch.
