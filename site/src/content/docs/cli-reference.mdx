...
| --- | --- | --- | --- |
| `--anthropic-admin-key-env` | - | `<name>` | Environment variable containing Anthropic Admin API key |
...
| `--source` | - | `<name>` | Filter by source id \(repeatable or comma-separated, supported sources \(7\): pi, codex, gemini, droid, opencode, claude, anthropic-api\) |
