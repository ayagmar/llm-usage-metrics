...
Generated from CLI version `0.7.1` via `scripts/generate-cli-reference.mjs`.
...
| `--pricing-overrides` | - | `<path>` | Path to a JSON file of per-model pricing overrides \(takes precedence over LiteLLM\) |
