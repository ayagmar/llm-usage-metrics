...
| `--provider` | - | `<name>` | Billing-provider filter \(normalized to billing entity; e.g. openai, anthropic, google\) \(daily, weekly, monthly, efficiency, optimize, trends, session only\) |
...
| `--top` | - | `<n>` | optimize: Show only the top N cheapest candidates \(positive integer\); session: Show only the top N sessions by cost \(positive integer\) |
