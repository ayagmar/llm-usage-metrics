...
| `--history` | - | - | include usage from files that no longer exist on disk \(from the local event store\) \(daily, weekly, monthly, efficiency, optimize, trends, session, wrapped only\) |
