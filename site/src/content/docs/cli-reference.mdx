...
| `--ignore-pricing-failures` | - | - | Continue without estimated costs when pricing cannot be loaded \(daily, weekly, monthly, efficiency, optimize, trends, session only\) |
...
| `--pricing-offline` | - | - | Use cached LiteLLM pricing only \(no network fetch\) \(daily, weekly, monthly, efficiency, optimize, trends, session only\) |
| `--pricing-overrides` | - | `<path>` | Path to a JSON file of per-model pricing overrides \(takes precedence over LiteLLM\) \(daily, weekly, monthly, efficiency, optimize, trends, session only\) |
| `--pricing-url` | - | `<url>` | Override LiteLLM pricing source URL \(daily, weekly, monthly, efficiency, optimize, trends, session only\) |
...
| `--share` | - | - | Write a share SVG image to the current directory \(daily, weekly, monthly, efficiency, optimize, trends only\) |
...
| `--timezone` | - | `<iana>` | Timezone for bucketing \(default: local system timezone\) \(daily, weekly, monthly, efficiency, optimize, trends, session only\) |
