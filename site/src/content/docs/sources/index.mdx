...
- `copilot-cli`: `~/.copilot/session-state` discovery for direct `*.jsonl` and `*/events.jsonl` (or `--copilot-cli-dir`)
- `copilot-vscode`: VS Code `workspaceStorage/*/chatSessions/*.json` discovery (or `--copilot-vscode-dir`)
