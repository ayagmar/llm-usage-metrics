...
Default LiteLLM pricing works without a warm cache because the CLI includes a
bundled LiteLLM snapshot. If you see this warning, the run succeeded from that
snapshot:
...
If pricing still fails, check for a custom `--pricing-url`, a bad
