...
- `--source-dir` supports only `pi`, `codex`, `gemini`, `droid`, `openclaw`, `claude`, `copilot`, and `amp`.
- `--pi-dir`, `--codex-dir`, `--gemini-dir`, `--droid-dir`, `--claude-dir`, `--copilot-dir`, and `--amp-dir` must point to valid directories.
