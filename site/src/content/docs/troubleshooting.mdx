...
- `--source-dir` supports only `pi`, `codex`, `gemini`, `droid`, `openclaw`, `claude`, `copilot`, `amp`, and `qwen`.
- `--pi-dir`, `--codex-dir`, `--gemini-dir`, `--droid-dir`, `--claude-dir`, `--copilot-dir`, `--amp-dir`, and `--qwen-dir` must point to valid directories.
